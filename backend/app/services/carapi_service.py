...
class CarAPIService:
    ...
    async def _make_request(self, endpoint: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"CarAPI request failed: {response.status_code} - {response.text}")
    ...
    async def get_makes(self, year: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            makes_objects = [{"make": make["name"], "make_id": make["id"]} for make in makes_data if "name" in make and "id" in make]
    ...
    async def get_models(self, year: int, make: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Convert to our expected format and sort alphabetically
            models_objects = [{"model": model["name"], "model_id": model["id"]} for model in models_data if "name" in model and "id" in model]
    ...
    async def get_trims(self, year: int, make: str, model: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            trims_objects = [{"trim": trim.get("trim", trim.get("name", "")), "trim_id": trim["id"]} for trim in trims_data if (trim.get("trim") or trim.get("name")) and "id" in trim]
