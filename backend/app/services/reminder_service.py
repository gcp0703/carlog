...
class ReminderService:
    ...
    def should_send_sms_reminder(self, user: User, current_date: date) -> bool:
        ...
        if user.sms_notification_frequency == "weekly":
            ...
            if current_date.day == 1:
                ...
                return (
                    last_sent.month != current_date.month
                    or last_sent.year != current_date.year
                )
    ...
    def should_send_maintenance_notification(
        self, user: User, current_date: date
    ) -> bool:
    ...
    async def send_sms_reminder(self, user: User) -> bool:
        ...
        try:
            if not user.phone_number:
                self.logger.warning(
                    f"User {user.id} has no phone number for SMS reminder"
                )
                return False
            ...
            if not vehicles:
                message = (
                    "Hi! It's time for your CarLog update. You don't have any vehicles "
                    "registered yet. Add a vehicle at carlog.piprivate.net to start tracking maintenance!"
                )
            elif len(vehicles) == 1:
                vehicle = vehicles[0]
                message = (
                    f"Hi! Time for your CarLog update. What's the current mileage on your "
                    f"{vehicle.year} {vehicle.brand} {vehicle.model}? Reply with just the number."
                )
            else:
                message = (
                    f"Hi! Time for your CarLog update. Reply with your current mileage for any "
                    f"of your {len(vehicles)} vehicles. Include vehicle name if you have multiple."
                )
            ...
            if message_sid:
                ...
                await self.neo4j_service.update_user(
                    user.id, {"last_update_request": datetime.utcnow()}
                )
    ...
    async def send_maintenance_notification(self, user: User) -> bool:
        ...
        try:
            ...
            if not vehicles:
                self.logger.info(
                    f"User {user.id} has no vehicles for maintenance notification"
                )
                return False
            ...
            if user.sms_notifications_enabled and user.phone_number:
                message = (
                    f"CarLog Maintenance Alert: Check your maintenance schedule at carlog.piprivate.net. "
                    f"You have {len(vehicles)} vehicle(s) that may need service soon."
                )
                message_sid = self.sms_service.send_sms(user.phone_number, message)
                if message_sid:
                    success = True
            ...
            if success:
                ...
                await self.neo4j_service.update_user(
                    user.id, {"last_maintenance_notification": datetime.utcnow()}
                )
            ...
            self.logger.error(
                f"Error sending maintenance notification to user {user.id}: {e}"
            )
            return False
